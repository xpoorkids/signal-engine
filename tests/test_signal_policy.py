...
def test_candidate_send_reasons_reject_synthetic_churn_without_independent_flow():
    eligible, reasons, confirmations = candidate_send_reasons(
        attention_score=0.20,
        creator_score=0.0,
        extra={
            "attention_metrics": {
                "dex_scan_persistent": True,
                "dex_scan_repeat_count": 8,
                "dex_scan_volume_delta_5m": 18.0,
                "independent_flow_confirmed": False,
                "unique_buyers_5m": 0,
                "burst_count_60s": 0,
                "tracked_wallet_hits": 0,
    ...
    eligible, reasons, confirmations = candidate_send_reasons(
        ...
        extra={
            "attention_metrics": {
                "dex_scan_persistent": True,
                "dex_scan_repeat_count": 8,
                "dex_scan_volume_delta_5m": 1_250.0,
                "independent_flow_confirmed": False,
                "unique_buyers_5m": 0,
                "burst_count_60s": 0,
                "tracked_wallet_hits": 0,
...
def test_viral_animal_theme_with_x_and_dex_flow_can_alert():
    eligible, reasons, confirmations = candidate_send_reasons(
        ...
        dex_summary={
            "liquidity_usd": 26_000.0,
            "volume_m5": 8_500.0,
            "txns_m5_buys": 19,
            "txns_m5_sells": 8,
            "price_change_m5": 4.0,
            "market_cap_usd": 420_000.0,
        },
    )
    ...
    assert "viral_x_momentum" in confirmations
    assert "market_support" in confirmations
    assert "entry_buy_pressure" in confirmations


def test_viral_theme_without_social_confirmation_does_not_alert():
    eligible, reasons, confirmations = candidate_send_reasons(
        attention_score=0.34,
        creator_score=0.0,
        extra={
            "attention_metrics": {
                "viral_theme_hits": ["debate"],
