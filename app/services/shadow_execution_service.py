...
def open_shadow_position(event) -> str | None:
    ...
    with _connect() as c:
        ...
        payload = {
            ...
            "pair_address": validation.get("pair_address"),
            "dex_id": validation.get("dex_id"),
            "entry_mid_price_usd": entry_mid_price,
        ...
        c.execute(
            ...
            (
                ...
                validation.get("pair_address"),
                validation.get("dex_id"),
                entry_mid_price or None,
                entry_exec_price,
                float(buy_quote.get("slippage_bps") or 0.0),
                float(((validation.get("sell_quote") or {}) if isinstance(validation.get("sell_quote"), dict) else {}).get("slippage_bps") or 0.0),
                _take_profit_pct(),
                _stop_loss_pct(),
                _max_hold_minutes(),
    ...
    logger.info(
        "[shadow-exec-open] token=%s position_id=%s signal_id=%s size_usd=%.2f entry_exec=%.8f qty=%.8f",
        event.token,
        position_id,
        signal_id or "",
        float(validation.get("intended_size_usd") or 0.0),
...
def _fetch_open_positions(limit: int = 25) -> list[dict[str, Any]]:
    _ensure_schema()
    with _connect() as c:
        rows = c.execute(
            """
            SELECT position_id, token, opened_ts, intended_size_usd, position_size_tokens,
...
async def refresh_open_position(position: dict[str, Any]) -> None:
    ...
    if ctx is None:
        ...
        return
    token_amount = float(position.get("position_size_tokens") or 0.0)
    intended_size_usd = float(position.get("intended_size_usd") or 0.0)
    ...
    age_minutes = max(0.0, (now - int(position.get("opened_ts") or now)) / 60.0)
    take_profit_pct = float(position.get("take_profit_pct") or _take_profit_pct())
    stop_loss_pct = float(position.get("stop_loss_pct") or _stop_loss_pct())
    max_hold_minutes = int(position.get("max_hold_minutes") or _max_hold_minutes())
    ...
    if exit_reason:
        ...
        logger.info(
            "[shadow-exec-mark] token=%s position_id=%s pnl_pct=%.2f pnl_usd=%.2f",
            token,
            position_id,
            pnl_pct,
            pnl_usd,
    ...
    while True:
        ...
        for position in positions:
            try:
                ...
                logger.exception("[shadow-exec-refresh-error] token=%s position_id=%s", position.get("token"), position.get("position_id"))
        await asyncio.sleep(_poll_seconds())
